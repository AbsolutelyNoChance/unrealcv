...
void UVisionBPLib::GetActorList(TArray<AActor *> &ActorList)
{
	...
	for (UObject *ActorObject : UObjectList)
	{
		...
		if (Actor->GetWorld() != FUnrealcvServer::Get().GetWorld())
			continue;
...
TArray<FVector> UVisionBPLib::SkinnedMeshComponentGetVertexArray(USkinnedMeshComponent *Component)
{
	...
	// stupid workaround to convert the arrays to the right formats
	auto a1 = TArray<FMatrix44f>();
	for (auto x : RefToLocals)
	{
		a1.Add(FMatrix44f(x));
	}
	auto a2 = TArray<FVector3f>();
	for (auto x : VertexArray)
	{
		a2.Add(FVector3f(x));
	}

	Component->CacheRefToLocalMatrices(a1);
	USkinnedMeshComponent::ComputeSkinnedPositions(Component, a2, a1, LODData, SkinWeightBuffer);
...
TArray<FVector> UVisionBPLib::StaticMeshComponentGetVertexArray(UStaticMeshComponent *StaticMeshComponent)
{
	...
	if (StaticMesh)
	{
		...
		uint32 NumLODLevel = RenderData->LODResources.Num();
		for (uint32 LODIndex = 0; LODIndex < NumLODLevel; LODIndex++)
		{
...
static EImageFormat GetJoyImageFormat(EJoyImageFormats JoyFormat)
{
	switch (JoyFormat)
	{
	case EJoyImageFormats::JPG:
		return EImageFormat::JPEG;
	case EJoyImageFormats::PNG:
		return EImageFormat::PNG;
	case EJoyImageFormats::BMP:
		return EImageFormat::BMP;
	case EJoyImageFormats::ICO:
		return EImageFormat::ICO;
	case EJoyImageFormats::EXR:
		return EImageFormat::EXR;
	case EJoyImageFormats::ICNS:
